 ...
 * 
 * builder.deepMostQuery("广东从化区温泉镇新田村", visitor);
 * StdRegion region1 = visitor.getResultDivision();
 * 
 * visitor.reset();
 * builder.deepMostQuery("湖南湘潭市湘潭县易俗河镇中南建材市场", visitor);
 * StdRegion region2 = visitor.getResultDivision();</pre>
 * 
...
public class RegionInterpreterVisitor implements TermIndexVisitor {
	protected final static Logger LOG = LoggerFactory.getLogger(RegionInterpreterVisitor.class);
	
	private static boolean isDebug = false;
	private int roundIndex = 0, visitIndex = 0;
	...
	public void startRound() {
		...
		if(isDebug && LOG.isDebugEnabled()){
			...
			for(int i=1; i<currentLevel; i++)
				sb.append("--------");
			sb.append("> ").append("[round-").append(roundIndex).append("-start]");
	 ...
	 */
	@Override
	public boolean visit(TermIndexEntry entry, int pos) {
		//找到最匹配的 被索引对象
		...
		if(isDebug && LOG.isDebugEnabled()) {
			...
			for(int i=1; i<currentLevel; i++)
				sb.append("--------");
			sb.append("> [visit-").append(visitIndex).append("] ").append(entry.getKey());
			sb.append(" : ");
			sb.append(acceptableItem.getValue().toString());
			LOG.info(sb.toString());
		}
		
		stack.push(acceptableItem);
		currentPos = pos;
		//刷新当前已经匹配上的省市区
		if(mostRegion!=null){
	 ...
	 */
	@Override
	public void endVisit(TermIndexEntry entry, int pos) {
		//打印调试信息
		if(isDebug && LOG.isDebugEnabled()) {
			StringBuilder sb = new StringBuilder();
			sb.append("|");
			for(int i=1; i<currentLevel; i++)
				sb.append("--------");
			sb.append("> [visit-").append(visitIndex).append("-end] ");
			LOG.info(sb.toString());
		}
	...
	public void endRound() {
		...
		if(isDebug && LOG.isDebugEnabled()) {
			...
			for(int i=1; i<currentLevel; i++)
				sb.append("--------");
			sb.append("> ").append("[round-").append(roundIndex).append("-end]");
			LOG.info(sb.toString());
		}
