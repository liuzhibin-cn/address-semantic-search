...
1. 地址不同于文章，文章是纯自然语言，而地址是格式规范性较强的短语；<br>
   在这里通用的分词技术显得不足，例如`海淀区`、`万达广场`不仅仅是一个词条，它们代表了一个`区县`、`POI`（即语义）。
   能够准确解析、识别地址的各个组成部分是地址相似度的关键工作。
2. 地址的特征并不是通过特征词的重复得以强化，而是通过特定组成部分强化的，例如道路名+门牌号、小区名等；<br>
   词频`TF`在地址相似度计算中没有意义，不同组成部分的权重设置对地址相似度的影响非常重要。
...
服务启动成功后，通过`http://localhost:8000/test/address/find/北京海淀区丹棱街18号创富大厦1106`访问。<br>
Demo服务运行效果如下：<br>
